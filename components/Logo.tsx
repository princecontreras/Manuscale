...
export const Logo: React.FC<{ className?: string, light?: boolean }> = ({ className = "", light = false }) => (
  <div className={`flex items-baseline select-none tracking-tight ${className}`}>
    <span className={`font-serif italic font-medium text-2xl ${light ? 'text-slate-300' : 'text-slate-500'}`}>
      Manu
    </span>
    <span className={`font-sans font-black text-2xl tracking-tighter ${light ? 'text-white' : 'text-slate-900'}`}>
      scale
    </span>
